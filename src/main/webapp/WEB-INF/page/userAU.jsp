...
<body style="background-color:#FFEBCD;">
	<form:form method="POST" id="forms" action="${pageContext.request.contextPath }/user/user" modelAttribute="user" >
		<div align="center">
			...
			<%--<p>地址:<form:input path="addrId" placeholder="请输入地址"/><p/>--%>
			<p>地址:
				<form:select path="addr.id" items="${addr}"></form:select>
			<p/>
			...
			<c:choose>
				<c:when test="${user.id != null && user.id != '' }">
					...
					<input id="userAddr" name="user.addrId" type="hidden" value="${page.t.user.addr.name }">
				</c:when>
				<c:otherwise>
